...
namespace CSV {
	...
	public:
		...
		int pushRangeImage(Mat& outPointCloud); //push out point cloud
		int pushPointCloud(vector<float>& outPointCloud);
		...
		void CsvGetStereoRectifyR1R2(Mat& M1, Mat& D1, Mat& M2, Mat& D2, Size imageSize,
			Mat& R, Mat& T, Mat& R1, Mat& R2);
		void CsvPointCloudToC1RangeImage(Mat& inPointCloud, Mat& out);

		void MedianFilter(const Mat& inM, Mat& outM, const int wnd_size);

		void CsvFilterGrayCode(const vector<Mat>& inList, Mat& outCode);
