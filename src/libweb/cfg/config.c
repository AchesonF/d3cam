...
/*
    Configure handlers when an webapp.config is not being used
 */
...
PUBLIC int maParseConfig(cchar *path)
{
    ...
    if (mprHasMemError()) {
        mprLog("error webapp memory", 0, "Memory allocation error when initializing");
...
static int parseFileInner(MaState *state, cchar *path)
{
    ...
    for (state->lineNumber = 1; state->file && (line = mprReadLine(state->file, 0, NULL)) != 0; state->lineNumber++) {
        ...
        if ((directive = mprLookupKey(directives, key)) == 0) {
            mprLog("error webapp config", 0, "Unknown directive \"%s\". At line %d in %s",
        ...
        if ((*directive)(state, key, value) < 0) {
            mprLog("error webapp config", 0, "Error with directive \"%s\". At line %d in %s",
    ...
    if (state->prev && state->file == state->prev->file) {
        mprLog("error webapp config", 0, "Unclosed directives in %s", state->filename);
...
static int addFilterDirective(MaState *state, cchar *key, cchar *value)
{
    ...
    if (httpAddRouteFilter(state->route, filter, extensions, HTTP_STAGE_RX | HTTP_STAGE_TX) < 0) {
        mprLog("error webapp config", 0, "Cannot add filter %s", filter);
...
static int addHandlerDirective(MaState *state, cchar *key, cchar *value)
{
    ...
    if (httpAddRouteHandler(state->route, handler, extensions) < 0) {
        mprLog("error webapp config", 0, "Cannot add handler %s", handler);
        return MPR_ERR_CANT_CREATE;
    }
    return 0;
}
...
static int authStoreDirective(MaState *state, cchar *key, cchar *value)
{
    if (httpSetAuthStore(state->auth, value) < 0) {
        mprLog("warn webapp config", 0, "The \"%s\" AuthStore is not available on this platform", value);
...
static int authTypeDirective(MaState *state, cchar *key, cchar *value)
{
    ...
    if (!smatch(type, "none")) {
        if (realm) {
            ...
            mprLog("warn webapp config", 0, "Must define an AuthRealm before defining the AuthType");
        }
        if (details) {
...
static int cacheDirective(MaState *state, cchar *key, cchar *value)
{
    ...
    for (option = stok(sclone(value), " \t", &tok); option; option = stok(0, " \t", &tok)) {
        ...
        if ((int) isdigit((uchar) *option)) {
            ...
            mprLog("error webapp config", 0, "Unknown Cache option '%s'", option);
            return MPR_ERR_BAD_SYNTAX;
        }
    }
...
static int chrootDirective(MaState *state, cchar *key, cchar *value)
{
    ...
    if (chdir(home) < 0) {
        mprLog("error webapp config", 0, "Cannot change working directory to %s", home);
        return MPR_ERR_CANT_OPEN;
    }
    if (state->route->flags & HTTP_ROUTE_NO_LISTEN) {
        /* Not running a web server */
        mprLog("info webapp config", 2, "Change directory to: \"%s\"", home);
    } else {
        if (chroot(home) < 0) {
            if (errno == EPERM) {
                mprLog("error webapp config", 0, "Must be super user to use chroot");
            } else {
                mprLog("error webapp config", 0, "Cannot change change root directory to %s, errno %d", home, errno);
            }
            return MPR_ERR_BAD_SYNTAX;
        }
        /*
        ...
        httpSetJail(home);
        mprLog("info webapp config", 2, "Chroot to: \"%s\"", home);
    }
...
static int crossOriginDirective(MaState *state, cchar *key, cchar *value)
{
    ...
    while ((option = maGetNextArg(tok, &tok)) != 0) {
        ...
        if (scaselessmatch(option, "origin")) {
            ...
            mprLog("error webapp config", 0, "Unknown CrossOrigin option %s", option);
            return MPR_ERR_BAD_SYNTAX;
        }
    }
#if KEEP
    if (smatch(route->corsOrigin, "*") && route->corsCredentials) {
        mprLog("error webapp config", 0, "CrossOrigin: Cannot use wildcard Origin if allowing credentials");
...
static int errorLogDirective(MaState *state, cchar *key, cchar *value)
{
    ...
    if (mprGetCmdlineLogging()) {
        mprLog("info webapp config", 4, "Already logging. Ignoring ErrorLog directive");
        return 0;
    }
    size = MAXINT;
    stamp = 0;
    ...
    for (option = maGetNextArg(sclone(value), &tok); option; option = maGetNextArg(tok, &tok)) {
        if (!path) {
            ...
            if (smatch(option, "size")) {
                ...
                mprLog("error webapp config", 0, "Unknown ErrorLog option %s", option);
            }
        }
    }
    if (size < (10 * 1000)) {
        mprLog("error webapp config", 0, "Size is too small. Must be larger than 10K");
        return MPR_ERR_BAD_SYNTAX;
    }
    if (path == 0) {
        mprLog("error webapp config", 0, "Missing filename");
        return MPR_ERR_BAD_SYNTAX;
    }
    ...
    if (mprStartLogging(path, MPR_LOG_DETAILED) < 0) {
        mprLog("error webapp config", 0, "Cannot write to ErrorLog: %s", path);
        return MPR_ERR_BAD_SYNTAX;
    }
...
static int headerDirective(MaState *state, cchar *key, cchar *value)
{
    ...
    if (scaselessmatch(cmd, "add")) {
        ...
        mprLog("error webapp config", 0, "Unknown Header directive operation: %s", cmd);
        return MPR_ERR_BAD_SYNTAX;
    }
...
static int includeDirective(MaState *state, cchar *key, cchar *value)
{
    ...
    /*
        Must use %S and not %P because the path is relative to the webapp.conf file and not to the route home
     */
...
static int limitFrameDirective(MaState *state, cchar *key, cchar *value)
{
    ...
    return 0;
#else
    mprLog("error webapp config", 0, "HTTP/2 is not enabled");
...
static int listenDirective(MaState *state, cchar *key, cchar *value)
{
    ...
    if (mprParseSocketAddress(address, &ip, &port, NULL, 80) < 0) {
        mprLog("error webapp config", 0, "Bad Listen directive: %s", address);
        return -1;
    }
    if (port == 0) {
        mprLog("error webapp config", 0, "Bad or missing port %d in Listen directive", port);
        return -1;
    }
    host = state->host;
...
static int listenSecureDirective(MaState *state, cchar *key, cchar *value)
{
    ...
    if (mprParseSocketAddress(address, &ip, &port, NULL, 443) < 0) {
        mprLog("error webapp config", 0, "Bad ListenSecure directive: %s", address);
        return -1;
    }
    if (port == 0) {
        mprLog("error webapp config", 0, "Bad or missing port %d in ListenSecure directive", port);
        return -1;
    }
    endpoint = httpCreateEndpoint(ip, port, NULL);
    if (smatch(multiple, "multiple")) {
    ...
    return 0;
#else
    mprLog("error webapp config", 0, "Configuration lacks SSL support");
    return -1;
#endif
}
...
static int loadModulePathDirective(MaState *state, cchar *key, cchar *value)
{
    ...
    /*
         Search path is: USER_SEARCH : exeDir : /usr/lib/webapp/bin
     */
    sep = MPR_SEARCH_SEP;
    path = stemplate(value, state->route->vars);
#ifdef CONFIG_VAPP_PREFIX
    path = sjoin(path, sep, mprGetAppDir(), sep, CONFIG_VAPP_PREFIX "/bin", NULL);
#endif
...
static int userToID(cchar *user)
{
    ...
    if ((pp = getpwnam(user)) == 0) {
        mprLog("error webapp config", 0, "Bad user: %s", user);
        return 0;
    }
    return pp->pw_uid;
}
...
static int groupToID(cchar *group)
{
    ...
    if ((gp = getgrnam(group)) == 0) {
        mprLog("error webapp config", 0, "Bad group: %s", group);
        return MPR_ERR_CANT_ACCESS;
    }
    return gp->gr_gid;
}
...
static int memoryPolicyDirective(MaState *state, cchar *key, cchar *value)
{
    ...
    if (scmp(policy, "restart") == 0) {
        ...
        mprLog("error webapp config", 0, "Unknown memory depletion policy '%s'", policy);
        return MPR_ERR_BAD_SYNTAX;
    }
...
static int rerouteDirective(MaState *state, cchar *key, cchar *value)
{
    ...
    if (state->enabled) {
        ...
        if ((route = httpLookupRoute(state->host, pattern)) != 0) {
            ...
            mprLog("error webapp config", 0, "Cannot open route %s", pattern);
            return MPR_ERR_CANT_OPEN;
        }
...
static int roleDirective(MaState *state, cchar *key, cchar *value)
{
    ...
    if (httpAddRole(state->auth, name, abilities) == 0) {
        mprLog("error webapp config", 0, "Cannot add role %s", name);
        return MPR_ERR_BAD_SYNTAX;
    }
    return 0;
}
...
static int sessionCookieDirective(MaState *state, cchar *key, cchar *value)
{
    ...
    for (option = maGetNextArg(options, &tok); option; option = maGetNextArg(tok, &tok)) {
        ...
        if (smatch(option, "visible")) {
            ...
            mprLog("error webapp config", 0, "Unknown SessionCookie option %s", option);
            return MPR_ERR_BAD_SYNTAX;
        }
    }
    return 0;
...
static int setConnectorDirective(MaState *state, cchar *key, cchar *value)
{
    if (httpSetRouteConnector(state->route, value) < 0) {
        mprLog("error webapp config", 0, "Cannot add handler %s", value);
        return MPR_ERR_CANT_CREATE;
    }
    return 0;
}
...
static int setHandlerDirective(MaState *state, cchar *key, cchar *value)
{
    ...
    if (httpSetRouteHandler(state->route, name) < 0) {
        mprLog("error webapp config", 0, "Cannot add handler %s", name);
        return MPR_ERR_CANT_CREATE;
    }
    return 0;
}
...
static int sslPreload(MaState *state, cchar *key, cchar *value)
{
    if (mprPreloadSsl(state->route->ssl, MPR_SOCKET_SERVER) < 0) {
        mprLog("error webapp config", 0, "Cannot preload SSL configuration");
        return MPR_ERR_BAD_SYNTAX;
    }
    return 0;
}

#endif /* CONFIG_COM_SSL */
...
PUBLIC int maTraceLogDirective(MaState *state, HttpTrace *trace, cchar *key, cchar *value)
{
    ...
    if (trace->flags & MPR_LOG_CMDLINE) {
        mprLog("info webapp config", 4, "Already tracing. Ignoring TraceLog directive");
        return 0;
    }
    for (option = maGetNextArg(sclone(value), &tok); option; option = maGetNextArg(tok, &tok)) {
        if (!path) {
            ...
            if (smatch(option, "anew")) {
                ...
                mprLog("error webapp config", 0, "Unknown TraceLog option %s", option);
            }
        }
    }
    ...
    if (path == 0) {
        mprLog("error webapp config", 0, "Missing TraceLog filename");
        return MPR_ERR_BAD_SYNTAX;
    }
    if (formatter) {
...
static int typesConfigDirective(MaState *state, cchar *key, cchar *value)
{
    ...
    if ((state->route->mimeTypes = mprCreateMimeTypes(path)) == 0) {
        mprLog("error webapp config", 0, "Cannot open TypesConfig mime file %s", path);
...
static int unloadModuleDirective(MaState *state, cchar *key, cchar *value)
{
    ...
    if ((module = mprLookupModule(name)) == 0) {
        mprLog("error webapp config", 0, "Cannot find module stage %s", name);
        return MPR_ERR_BAD_SYNTAX;
    }
    if ((stage = httpLookupStage(module->name)) != 0 && stage->match) {
        mprLog("error webapp config", 0, "Cannot unload module %s due to match routine", module->name);
        return MPR_ERR_BAD_SYNTAX;
    } else {
...
static int userDirective(MaState *state, cchar *key, cchar *value)
{
    ...
    if (httpAddUser(state->auth, name, password, roles) == 0) {
        mprLog("error webapp config", 0, "Cannot add user %s", name);
        return MPR_ERR_BAD_SYNTAX;
    }
    return 0;
}
...
static int virtualHostDirective(MaState *state, cchar *key, cchar *value)
{
    ...
    if (state->enabled) {
        ...
        if (value) {
            ...
            mprLog("error webapp config", 0, "Missing virtual host endpoints");
            return MPR_ERR_BAD_SYNTAX;
        }
    }
    return 0;
        ...
        if (state->endpoints && *state->endpoints) {
            for (addresses = sclone(state->endpoints); (address = stok(addresses, " \t,", &tok)) != 0 ; addresses = tok) {
                if (mprParseSocketAddress(address, &ip, &port, NULL, -1) < 0) {
                    mprLog("error webapp config", 0, "Bad virtual host endpoint %s", address);
                    return MPR_ERR_BAD_SYNTAX;
                }
                if ((endpoint = httpLookupEndpoint(ip, port)) == 0) {
                    mprLog("error webapp config", 0, "Cannot find listen directive for virtual host %s", address);
                    return MPR_ERR_BAD_SYNTAX;
                } else {
...
#if CONFIG_DEBUG
    } else if (scaselessmatch(key, "CONFIG_DEBUG")) {
        result = CONFIG_DEBUG;
#endif
...
PUBLIC bool maTokenize(MaState *state, cchar *line, cchar *fmt, ...)
{
    ...
    if (!httpTokenizev(state->route, line, fmt, ap)) {
        mprLog("error webapp config", 0, "Bad \"%s\" directive at line %d in %s, line: %s %s",
...
static int addUpdate(MaState *state, cchar *name, cchar *details, int flags)
{
    if (httpAddRouteUpdate(state->route, name, details, flags) < 0) {
        mprLog("error webapp config", 0, "Bad \"%s\" directive at line %d in %s, line: %s %s %s",
...
{
    if (state->prev == 0) {
        mprLog("error webapp config", 0, "Too many closing blocks.\nAt line %d in %s\n\n", state->lineNumber, state->filename);
    }
...
static int configError(MaState *state, cchar *key)
{
    mprLog("error webapp config", 0, "Error in directive \"%s\", at line %d in %s", key, state->lineNumber, state->filename);
    return MPR_ERR_BAD_SYNTAX;
}
...
PUBLIC int maWriteAuthFile(HttpAuth *auth, char *path)
{
    ...
    if ((file = mprOpenFile(tempFile, O_CREAT | O_TRUNC | O_WRONLY | O_TEXT, 0444)) == 0) {
        mprLog("error webapp config", 0, "Cannot open %s", tempFile);
        return MPR_ERR_CANT_OPEN;
    }
    ...
    if (rename(tempFile, path) < 0) {
        mprLog("error webapp config", 0, "Cannot create new %s", path);
...
PUBLIC int maLoadModule(cchar *name, cchar *libname)
{
    ...
    path = libname ? libname : sjoin("libmod_", name, CONFIG_SHOBJ, NULL);
