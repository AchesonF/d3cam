...
PUBLIC Http *httpCreate(int flags)
{
    ...
    http->startLevel = 2;
    http->localPlatform = slower(sfmt("%s-%s-%s", CONFIG_OS, ME_CPU, CONFIG_PROFILE));
...
PUBLIC int httpSetUserAccount(cchar *newUser)
{
    ...
    http = HTTP;
    if (smatch(newUser, "HTTP") || smatch(newUser, "WEBAPP")) {
...
PUBLIC int httpSetGroupAccount(cchar *newGroup)
{
    ...
    http = HTTP;
    if (smatch(newGroup, "HTTP") || smatch(newGroup, "WEBAPP")) {
