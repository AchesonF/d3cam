...
/*
    ...
    NOTE: this is heuristic and not perfect. Users can define the template via the httpSetTemplate API or in webapp via the
...
{
    /*
        Add the route to the owning host. When using an Webapp configuration file, the order of route finalization
...
static void definePathVars(HttpRoute *route)
{
    ...
    mprAddKey(route->vars, "PRODUCT", sclone(CONFIG_NAME));
    mprAddKey(route->vars, "OS", sclone(CONFIG_OS));
    mprAddKey(route->vars, "VERSION", sclone(CONFIG_VERSION));
