...
#undef OCSP_RESPONSE
    ...
    #define CONFIG_MPR_SSL_HANDSHAKES 0     /* Defaults to infinite */
#endif
...
static int configOss(MprSsl *ssl, int flags, char **errorMsg)
{
     ...
     */
    #if defined(CONFIG_MPR_SSL_TICKET)
        if (CONFIG_MPR_SSL_TICKET) {
...
#if defined(CONFIG_MPR_SSL_HANDSHAKES)
    cfg->maxHandshakes = CONFIG_MPR_SSL_HANDSHAKES;
#endif
...
#if defined(CONFIG_MPR_SSL_CACHE)
    /*
        Set the number of sessions supported. Default in OpenSSL is 20K.
     */
    SSL_CTX_sess_set_cache_size(ctx, CONFIG_MPR_SSL_CACHE);
#else
