...
static cchar *getModuleName(HttpRoute *route, cchar *kind, cchar *target)
{
    ...
    return mprJoinPathExt(mprJoinPaths(route->home, cacheDir, cache, NULL), CONFIG_SHOBJ);
}
...
static int espLoadModule(HttpRoute *route, MprDispatcher *dispatcher, cchar *kind, cchar *source, cchar **errMsg, bool *loaded)
{
    ...
    *errMsg = "";
    ...
    module = mprJoinPathExt(mprJoinPaths(route->home, cacheDir, cache, NULL), CONFIG_SHOBJ);
                    ...
                    if (!mprPathExists(documents, X_OK)) {
                        ...
                        if (!esp->hostedDocuments && mprPathExists("install.conf", R_OK)) {
                            /*
                                ...
                                When Webapp switches to webapp.json, then just it should be loaded with pak.json
                             */
                            char *output;
                            bool yielding = mprSetThreadYield(NULL, 0);
                            if (mprRun(NULL, "webapp --show-documents", NULL, (char**) &output, NULL, 5000) == 0) {
