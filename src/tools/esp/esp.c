...
//            if (!mprIsVersionAcceptable(CONFIG_VERSION, criteria)) {
//                fail("ESP %s is not acceptable for this application which requires ESP %s", CONFIG_VERSION, criteria);
//            }
        }
    }
...
static void generate(int argc, char **argv)
{
    ...
    if (smatch(kind, "webapp") || smatch(kind, "webapp.conf")) {
        generateItem("webapp");
...
static void init(int argc, char **argv)
{
    ...
    if (!mprPathExists("esp.json", R_OK)) {
        ...
        data = sjoin("/*\n" \
            ...
            "   See esp/tree/dev/samples/config for a commented sample.\n" \
            " */\n", data, NULL);
...
static void compileFile(HttpRoute *route, cchar *source, int kind)
{
    ...
    app->module = mprNormalizePath(sfmt("%s/%s%s", cacheDir, app->cacheName, CONFIG_SHOBJ));
    ...
    if (!app->combineFile) {
        ...
        if (eroute->linkCmd) {
             ...
             */
            mprDeletePath(mprJoinPathExt(mprTrimPathExt(app->module), CONFIG_OBJ));
        }
...
static void compileCombined(HttpRoute *route)
{
    ...
    if (mprGetHashLength(app->build) > 0) {
        ...
        app->module = mprNormalizePath(sfmt("%s/%s%s", httpGetDir(route, "CACHE"), name, CONFIG_SHOBJ));
...
static void usageError()
{
    ...
    paks = getCachedPaks();
    if (paks) {
        mprEprintf("  Local Paks: (See also /catalog/)\n%s\n", paks);
    }
    app->error = 1;
}
